...
stats-line 的用户面是**组件序列**:内置数据组件(`counts`/`llm`/`tools`/`ttft`/`tps`/`tokens`/`cost`)、分隔符(`sep`,small '·' / big '|')与自定义模板组件(`custom`,`{placeholder}` 插值,占位符为预格式化显示值)。数据不可得的组件不渲染(包括未装 session-cost 时的 cost),**分隔符自动收敛**(边缘删除、相邻留大)——这就是声明式的条件显隐;设置 GUI 里的卡片是可拖拽编排器(拖动排序、点分隔符切大小),带示例数据实时预览。`css` 是高级区逃生舱。边界纪律:自定义模板只插值、无逻辑语法;新组件种类去改 client。
