...
interface SettingsScopeService {
  bind(opts: { namespace: string }): SettingsController
}
...
interface SlotsService {
  ...
  register(desc: { name: string; key?: string; locale?: string }, component: (props: CellProps) => React.ReactElement | null): () => void
}

interface ClientContext {
  effect<T>(fn: () => T, ...args: unknown[]): T
  get(name: string): unknown
}

let settingsController: SettingsController | null = null

function useSettingsSnapshot(): SettingsSnapshot | null {
  const ctrl = settingsController
  return React.useSyncExternalStore(
    (fn) => (ctrl !== null ? ctrl.subscribe(fn) : () => {}),
    () => (ctrl !== null ? ctrl.getSnapshot() : null),
  )
}

// ── 设置卡片(settings.plugin.item,key = session-cost) ─────────────────
...
function draftFromDoc(doc: Record<string, unknown>): Draft {
...
function SessionCostCard(_props: CellProps): React.ReactElement | null {
  const snap = useSettingsSnapshot()
  const [open, setOpen] = React.useState(false)
  const [draft, setDraft] = React.useState<Draft | null>(null)
  const t = (_props.ctx as { t?: Translate } | undefined)?.t ?? ((key: DictKey) => en[key])
  const doc = snap?.value ?? {}
  const value = draft ?? draftFromDoc(doc)
  const dirty = draft !== null
  const writable = snap?.writable !== false
  const edit = (patch: Partial<Draft>): void => setDraft({ ...value, ...patch })
  ...
  const field = (key: keyof Draft, labelKey: DictKey, errKey: DictKey | null, valid: boolean): React.ReactElement =>
    h('div', { className: 'scc-field', key }, [
      h('label', { className: 'scc-label', key: 'l' }, t(labelKey)),
      h('input', {
        className: 'scc-input',
        key: 'i',
        type: 'text',
        ...
        onChange: (e: React.ChangeEvent<HTMLInputElement>) => edit({ [key]: e.target.value }),
      }),
      !valid && errKey !== null ? h('div', { className: 'scc-error', key: 'e' }, t(errKey)) : null,
    ])

  const body = open
    ? h('div', { className: 'scc-body', key: 'body' }, [
        ...
        h('div', { className: 'scc-footer', key: 'foot' }, [
          h('button', { className: 'scc-discard', key: 'd', type: 'button', disabled: !dirty, onClick: () => setDraft(null) }, t('cardDiscard')),
          h('button', { className: 'scc-save', key: 's', type: 'button', disabled: !dirty || !rateValid || !decValid || !writable, onClick: save }, t('cardSave')),
        ]),
      ])
    : null

  return h('li', { className: open ? 'scc-card scc-open' : 'scc-card' }, [
    h(
      'button',
      { className: 'scc-header', key: 'head', type: 'button', 'aria-expanded': open, onClick: () => setOpen(!open) },
      [
        h('span', { className: 'scc-headtext', key: 'txt' }, [
          h('span', { className: 'scc-name', key: 'n' }, t('cardTitle')),
          h('span', { className: 'scc-desc', key: 'd' }, t('cardDescription')),
        ]),
        dirty ? h('span', { className: 'scc-pending', key: 'pd' }, t('cardUnsaved')) : null,
        h(
          'svg',
          { className: 'scc-chevron', key: 'ch', width: 14, height: 14, viewBox: '0 0 14 14', fill: 'none', 'aria-hidden': true },
...
export function apply(ctx: ClientContext): void {
  ...
  if (settingsScope !== undefined) settingsController = settingsScope.bind({ namespace: 'session-cost' })
